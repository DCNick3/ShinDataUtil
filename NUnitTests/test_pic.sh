...
do
  ...
  diff <(find "$TDIR/orig_pic" -type f -exec md5sum {} + | sort -k 2 | cut -f1 -d" ") \
    <(find "$TDIR/reenc_pic" -type f -exec md5sum {} + | sort -k 2 | cut -f1 -d" ")
done <   <(find "$DATARAWDIR" -name '*.pic' -xtype f -print0)
