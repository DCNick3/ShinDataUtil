...
{
    public static unsafe class ShinOpusRemuxer
    {
        // The reference implementation is not so good for multi-threaded environment, so I've created my own =)
        // TODO: reduce excessive copying of data back and forth (it can be done in no-coping manner)
        
        private static readonly Random Rnd = new Random();
        
        static ShinOpusRemuxer()
        {
            Trace.Assert(BitConverter.IsLittleEndian);
        }

        private static (int packetSize, int whatever) ReadNintendoPacketHeader(ReadOnlySpan<byte> p)
        {
            return ((p[0] << 24) | (p[1] << 16) | (p[2] << 8) | p[3],
                (p[4] << 24) | (p[5] << 16) | (p[6] << 8) | p[7]);
        }

        //public static ConcurrentDictionary<string, (int, int)> _knownDecodeParameters = new ConcurrentDictionary<string, (int, int)>();
        
        public static void Remux(ReadOnlySpan<byte> file, Stream output, string basePath)
        {
            var header = MemoryMarshal.Read<NxaHeader>(file);
            var data = file[sizeof(NxaHeader)..];

            //if (Math.Abs(header.samples_length_another - header.samples_length) > 2)
            //    Debugger.Break();

            var bufferSizeBase = (header.probably_frame_size + header.sample_rate / 10 - 1) /
                                   header.probably_frame_size;
            var inputBufferSize = bufferSizeBase * header.input_buffer_multiplier;
            var outputBufferSize = bufferSizeBase * header.probably_frame_size * header.channel_count * 2;
            if (header.sample_rate != 48000)
                Debugger.Break();

            //_knownDecodeParameters[basePath] = (header.probably_frame_size, header.input_buffer_multiplier);
            //var input_size = 
            
            var streamId = Rnd.Next();
            
            var idHeader = MakeIdHeader((byte) header.channel_count, header.sample_rate);
            var tagsHeader = MakeOpusCommentHeader();
            
            WritePage(streamId, true, false, 0, 
                0, new [] {idHeader}, output);
            
            WritePage(streamId, false, false, 1, 
                0, new [] {tagsHeader}, output);
            ...
            while (!data.IsEmpty)
            {
                var (packetSize, whatever) = ReadNintendoPacketHeader(data);
                data = data[8..];
                var packetData = data[..packetSize];
                data = data[packetSize..];

                var lacingSizeChange = 1 + packetData.Length / 255;

                if (lacingSize + lacingSizeChange > 255)
                {
                    WritePage(streamId, false, data.IsEmpty, pageSeq, granulePosition, messageList, output);
                    messageList.Clear();
                    lacingSize = lacingSizeChange;
                    pageSeq++;
                }
                else
                    lacingSize += lacingSizeChange;

                messageList.Add(packetData.ToArray());
                granulePosition += DeterminePacketPcmLength(packetData);
            }
            
            if (lacingSize > 0)
                WritePage(streamId, false, true, pageSeq, granulePosition, messageList, output);
        }

        private static void WritePage(int streamId, bool beginOfStream, bool endOfStream, int pageSeqNumber, 
            long granulePosition, IReadOnlyCollection<byte[]> entries, Stream outStream)
        {
            using var ms = new MemoryStream();
            using var br = new BinaryWriter(ms);
            
            br.Write(0x5367674f); // OggS
            br.Write((byte)0); // version
            br.Write((byte)((beginOfStream ? 0x2 : 0) | (endOfStream ? 0x4 : 0))); // bos
            br.Write(granulePosition); // granule_position
            br.Write(streamId);
            br.Write(pageSeqNumber); // page_sequence_number
            br.Write(0); // crc
            
            br.Write((byte)entries.Select(_ => 1 + _.Length / 255).Sum()); // # of entries
            ...
            var crc = Crc32.Get(ms.GetBuffer()[..(int)ms.Length]);

            br.Seek(22, SeekOrigin.Begin);
            br.Write(crc);
        ...
        private static int DeterminePacketPcmLength(ReadOnlySpan<byte> packetData)
        {
            var firstByte = packetData[0];
            var config = firstByte >> 0x3;
            var c = firstByte & 0x3;
            var baseLength = config switch
            {
                16 => (5 * 48 / 2),
                20 => (5 * 48 / 2),
                24 => (5 * 48 / 2),
            ...
            var secondByte = packetData[1];
            var m = secondByte & 0x3f;

            return baseLength * m;
        }

        private static byte[] MakeOpusCommentHeader()
        {
        ...
        private static byte[] MakeIdHeader(byte channelCount, uint inputSampleRate)
        {
            var idHeader = new OpusIdentificationHeader
            {
                magic = 0x646165487375704f, // OpusHead
                version = 1,
            ...
            var idHeaderBuffer = new byte[Marshal.SizeOf<OpusIdentificationHeader>()];
            Marshal.Copy(new IntPtr(&idHeader), idHeaderBuffer, 0, sizeof(OpusIdentificationHeader));
            return idHeaderBuffer;
        }

        private struct NxaHeader
        {
#pragma warning disable 649
            ...
            public ushort input_buffer_multiplier;
            public ushort probably_frame_size;
            public ushort pre_skip;
            public uint samples_length_another;
            public uint repeat_offset_samples;
            public uint samples_length;
            // ReSharper restore InconsistentNaming
            public fixed byte whatever[12];
#pragma warning restore 649
        ...
        private struct OpusIdentificationHeader
        {
            ...
            public ushort outputGain;
            public byte mappingFamily;
            // mapping is not actually used, so it is ommitted
        }

        private class Crc32
        {
            // Copied from libogg
            static readonly uint[,] CrcLookup =
            {
                {
            ...
            public static uint Get(ReadOnlySpan<byte> buffer, uint crc = 0)
            {
                var size = buffer.Length;

                while (size >= 8)
                {
                    crc ^= (uint) ((buffer[0] << 24) | (buffer[1] << 16) | (buffer[2] << 8) |
                                   buffer[3]);

                    crc = CrcLookup[7, crc >> 24] ^ CrcLookup[6, (crc >> 16) & 0xFF] ^
                          CrcLookup[5, (crc >> 8) & 0xFF] ^ CrcLookup[4, crc & 0xFF] ^
                          CrcLookup[3, buffer[4]] ^ CrcLookup[2, buffer[5]] ^
                          CrcLookup[1, buffer[6]] ^ CrcLookup[0, buffer[7]];
                ...
                foreach (var b in buffer)
                    crc = (crc << 8) ^ CrcLookup[0, ((crc >> 24) & 0xff) ^ b];
